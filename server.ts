...
const QUOTA_COOLDOWN_MS = 60000; // Check again after 60s in case user replenished credits

function getAI(): GoogleGenAI | null {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey || apiKey.trim() === "" || apiKey === "dummy-key" || apiKey === "MY_GEMINI_API_KEY") {
    return null;
  }
  if (!aiClient) {
...
async function generateContentWithRetry(options: {
  ...
  const now = Date.now();
  if (isQuotaExhausted && now - lastQuotaCheckTime < QUOTA_COOLDOWN_MS) {
    throw new Error("Prepayment credits are depleted on the active project. Sovereign fallback engines engaged.");
  }
  ...
  const ai = getAI();
  if (!ai) {
    throw new Error("GEMINI_API_KEY is not configured or invalid. Using sovereign offline heuristic engines.");
  }
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (response && response.text) {
          isQuotaExhausted = false; // Reset flag on successful call
          return response;
        }
      } catch (err: any) {
        lastError = err;
        const errStr = String(err?.message || err);
        const isQuotaDepleted = errStr.includes("RESOURCE_EXHAUSTED") || errStr.includes("prepayment credits are depleted") || errStr.includes("quota");
        
        if (isQuotaDepleted) {
          isQuotaExhausted = true;
          lastQuotaCheckTime = Date.now();
          break; // Avoid calling subsequent models when project credit is exhausted
        }
...
`;
  ...
  const handleComplaintsList = (req: express.Request, res: express.Response) => {
  ...
  app.get("/api/ministerial-complaints", handleComplaintsList);
  app.get("/api/brics-complaints", handleComplaintsList);
  app.get("/api/complaints", handleComplaintsList);
...
`;
  ...
  // Explicit JSON 404 for unhandled /api/* requests so client never gets HTML for API calls
  app.all("/api/*", (req, res) => {
    res.status(404).json({
      success: false,
